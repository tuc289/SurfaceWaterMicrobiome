...
## Plot a PCoA plot with samples color-coded based on the pathogen presence.
# Bacterial communities.
set.seed(420)
PCoA_total_16s = ordinate(phyloseq1_family, "PCoA", "Unifrac", weighted = TRUE)
PCoA_plot_16s_sal = plot_ordination(phyloseq1_family, PCoA_total_16s, color="pos_s",shape="Sampletype")  
sal <- PCoA_plot_16s_sal + geom_point(size=3) + labs(x= "PC 1 [34.6%]", y= "PC 2 [14.9%]") + scale_color_manual(values= c("Red", "Blue")) +theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
lm <- PCoA_plot_16s_lm + geom_point(size=3) + labs(x= "PC 1 [34.6%]", y= "PC 2 [14.9%]") + scale_color_manual(values= c("Red", "Blue","Black")) +theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())

pp1 <- plot_grid(sal+ theme(legend.position="none")
                 , lm+ theme(legend.position="none"), labels=c("A","B"))
  
lm_legend <- lm + 
  scale_color_manual(name="Pathogen", breaks=c("0","1","MS Lost"), labels=c("Negative", "Positive", "Not tested"),values= c("Red", "Blue","Black")) +
...
set.seed(421)
PCoA_total_ITS = ordinate(phyloseq2_family, "PCoA", "Unifrac", weighted = TRUE)
PCoA_plot_ITS_sal = plot_ordination(phyloseq2_family, PCoA_total_ITS, color="pos_s",shape="Sampletype")  
sal2 <- PCoA_plot_ITS_sal + geom_point(size=3) + labs(x= "PC 1 [34.6%]", y= "PC 2 [14.9%]") + scale_color_manual(values= c("Red", "Blue")) +theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
lm2 <- PCoA_plot_ITS_lm + geom_point(size=3) + labs(x= "PC 1 [34.6%]", y= "PC 2 [14.9%]") + scale_color_manual(values= c("Red", "Blue","Black")) +theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
lm_legend <- lm2 
lm_legend <- lm2 + 
  scale_color_manual(name="Pathogen", breaks=c("0","1","MS Lost"), labels=c("Negative", "Positive", "MS Lost"),values= c("Red", "Blue","Black")) +
...
# Run PERMANOVA to test for associations between microbial community composition and pathogen presence.
set.seed(114)
adonis_sal_Sediment1 <- adonis(phyloseq::distance(Sediment1,method="unifrac",weighted=TRUE) ~ pos_s
                               , data= as(sample_data(Sediment1), "data.frame"))
adonis_sal_Sediment2 <- adonis(phyloseq::distance(Sediment2,method="unifrac",weighted=TRUE) ~ pos_s
                               , data= as(sample_data(Sediment2), "data.frame"))
adonis_sal_Water1 <- adonis(phyloseq::distance(Water1,method="unifrac",weighted=TRUE) ~ pos_s
                            , data= as(sample_data(Water1), "data.frame"))
adonis_sal_Water2 <- adonis(phyloseq::distance(Water2,method="unifrac",weighted=TRUE) ~ pos_s
...
set.seed(116)
adonis_lm_Sediment1 <- adonis(phyloseq::distance(Sediment1,method="unifrac",weighted=TRUE) ~ lm
                              , data= as(sample_data(Sediment1), "data.frame"))
adonis_lm_Sediment2 <- adonis(phyloseq::distance(Sediment2,method="unifrac",weighted=TRUE) ~ lm
                              , data= as(sample_data(Sediment2), "data.frame"))
adonis_lm_Water1 <- adonis(phyloseq::distance(Water1,method="unifrac",weighted=TRUE) ~ lm
                           , data= as(sample_data(Water1), "data.frame"))
adonis_lm_Water2 <- adonis(phyloseq::distance(Water2,method="unifrac",weighted=TRUE) ~ lm
                           , data= as(sample_data(Water2), "data.frame"))
